...
var api_key = 'key-02375877809ad8f5c79c4d97f5d2fe2c';

//Your domain, from the Mailgun Control Panel
var domain = 'mg.agenceduweb.be';

//Your sending email address
var from_who = 'postmaster@mg.agenceduweb.be';

// views jade folder
var jadefolder = "jademailgun/";
...
app.get('/', function(req, res) {
    ...
    res.render(jadefolder+'index', function(err, html) {
        if (err) {
...
app.get('/submit/:mail', function(req,res) {
    ...
    mailgun.messages().send(data, function (err, body) {
        ...
        if (err) {
            res.render(jadefolder+'error', { error : err});
        ...
        //Else we can greet    and leave
            ...
            res.render(jadefolder+'submitted', { email : req.params.mail });
            console.log(body);
        }
    });
...
//For the sake of this tutorial you need to create a mailing list on Mailgun.com/cp/lists and put its address below
    mailgun.lists('axelfiollemail@mg.agenceduweb.be').members().add({ members: members, subscribed: true }, function (err, body) {
      console.log(body);
      if (err) {
...
app.get('/invoice/:mail', function(req,res){
    ...
    mailgun.messages().send(data, function (error, body) {
        if (error) {
            res.render(jadefolder+'error', {error: error});
        }
            else {
